...
Redis 会把设置了过期时间的key放入一个独立的字典里，再key不哦器是并不会立刻删除它。Redis会通过如下两种策略，来删除过期的key：
...
当Redis占用内存超出最大限制（maxmemory）时，可采用如下策略（maxmemory-policy），让Redis淘汰一些数据，以天恩出空间继续提供读写服务：

* noeviction：对可能导致增大内存的名利返回错误（大多数写命令，DEL除外）；----------默认。
* volatile-ttl:再设置了过期时间的key中，选择生于寿命（TTL）最短的key，将其淘汰；
