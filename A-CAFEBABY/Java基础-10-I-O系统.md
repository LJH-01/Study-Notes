...
`read()` 和 `write()` 调用得到了极大的简化，因为许多工作细节都由缓冲区完成了。 `clear()` --可以读和 `flip()` --可以写方法用于让缓冲区在读和写之间切换。 
...
- Greg Travis 在他的 "[*JDK 1.4                             Tutorial”* ](http://www.manning.com/travis/) (Manning 出版社，2002年3月)一书中仔细研究了 NIO。 
